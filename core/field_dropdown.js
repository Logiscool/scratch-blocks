 ...
 * @constructor
 */
Blockly.FieldDropdown = function(menuGenerator, opt_valueProvider, opt_validator) {
  this.menuGenerator_ = menuGenerator;
  this.valueProvider_ = opt_valueProvider;
  //this.trimOptions_();
...
Blockly.FieldDropdown.prototype.onItemSelected = function(menu, menuItem) {
  ...
  if (this.sourceBlock_) {
    ...
    var result = this.callValidator(value);
    if(result && result.then) {
      result.then((value) => this.setValue(value))
    }
    else {
      value = result;
    }
...
Blockly.FieldDropdown.prototype.provideValue_ = function(fallbackValue) {
  if(this.valueProvider_) {
    var result = this.valueProvider_(fallbackValue);
    if(result && result.then) {
      return result
    }
    else {
...
Blockly.FieldDropdown.prototype.setValue = function(newValue) {
  ...
  if(this.sourceBlock_ && Blockly.Events.isEnabled()) {
    this.provideValue_(newValue).then((newRealValue) => {
      Blockly.Events.fire(new Blockly.Events.BlockChange(
        this.sourceBlock_, 'field', this.name, this.realValue_, newRealValue));
      this.realValue_ = newRealValue;
    })
  }
